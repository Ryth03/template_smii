            ...
            <div class="py-6">
                <div class="px-6">
                    <div class="box-header flex flex-col justify-start items-center">
                        <h3 class="box-title m-0 text-2xl">G1 | <span class="text-orange-500">Ambient</span></h3>
                        <h3 class="box-title m-0 text-xl">4,132 PP</h3>
                    </div>
                </div>
    ...
    <div style="grid-column: span 5/span 5;">
        <div class="card border-2 border-success rounded-2xl ">
                        ...
                        <ul class="m-0" style="list-style: none;">
                            <li class="dropdown">
                                <button id="dateDisplay" class="waves-effect waves-light btn btn-outline dropdown-toggle btn-md"
                                    ...
                                    <span class="text-2xl">{{ date('d F Y') }} <!-- Menampilkan tanggal hari ini --></span>
                                </button>
            ...
            <div class="box-body grid grid-cols-5" style="column-gap: 0.75rem /* 12px */;">
                <div class="">
                    <div class="box-header flex flex-col justify-start items-center">
                        <h3 class="box-title m-0 text-2xl">G2 | <span class="text-blue-500">16°C</span></h3>
                        <h3 class="box-title m-0 text-xl">1,000 PP</h3>
                    </div>
                ...
                <div class="">
                    <div class="box-header flex flex-col justify-start items-center">
                        <h3 class="box-title m-0 text-2xl">G2 | <span class="text-green-500">25°C</span></h3>
                        <h3 class="box-title m-0 text-xl">1,000 PP</h3>
                    </div>
                ...
                <div class="">
                    <div class="box-header flex flex-col justify-start items-center">
                        <h3 class="box-title m-0 text-2xl">G3 | <span class="text-green-500">25°C</span></h3>
                        <h3 class="box-title m-0 text-xl">1,000 PP</h3>
                    </div>
                ...
                <div class="">
                    <div class="box-header flex flex-col justify-start items-center">
                        <h3 class="box-title m-0 text-2xl">G3 | <span style="color: #FB923C;">Ambient</span></h3>
                        <h3 class="box-title m-0 text-xl">7,864 PP</h3>
                    </div>
                ...
                <div class="grid grid-cols-2 items-start">
                        @php
                            ...
                            $bawah = rand(15, 42);
                        @endphp
                    <div class="flex justify-center self-center text-2xl col-span-2 " style="">Act. Temp.</div>
                    <div class="flex justify-center text-2xl">Atas</div>
                    <div class="flex"><p class="px-1 text-3xl {{ $atas >= 38 ? 'text-white' : ($atas >= 32 ? 'text-black' : '') }}" style="background-color:  {{ $atas >=38 ? '#FF5E5C' : ($atas >= 32 ? '#EFA70B' : '') }}; ">{{$atas}}°C</p></div>
                    <div class="flex justify-center text-2xl">Tengah</div>
                    <div class="flex"><p class="px-1 text-3xl {{ $tengah >= 38 ? 'text-white' : ($tengah >= 32 ? 'text-black' : '') }}" style="background-color:  {{ $tengah >=38 ? '#FF5E5C' : ($tengah >= 32 ? '#FFB95C' : '') }}; ">{{$tengah}}°C</p></div>
                    <div class="flex justify-center text-2xl">Bawah</div>
                    <div class="flex"><p class="px-1 text-3xl {{ $bawah >= 38 ? 'text-white' : ($bawah >= 32 ? 'text-black' : '') }}" style="background-color:  {{ $bawah >=38 ? '#FF5E5C' : ($bawah >= 32 ? '#FFB95C' : '') }}; ">{{$bawah}}°C</p></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="box">
            <div class="box-body analytics-info">
                <div class="flex py-2" style="width:100%; justify-content:space-between;">
                    <div class="text-2xl font-medium">DISPATCH</div>
                    <div>
                        <!-- Daftar Tahun -->
        ...
        <div class="card rounded-2xl">
            <div class="box-header flex justify-between items-center">
                <div class="flex flex-row flex-wrap">
                    <h4 class="box-title text-2xl font-medium">Daily Dispatch</h4>
                    <h4 class="box-title text-2xl font-medium">(in Tonnage)</h4>
                </div>
                <ul class="m-0" style="list-style: none;">
                    <li class="dropdown">
                        <button id="dateDisplay2" class="waves-effect waves-light btn btn-outline dropdown-toggle btn-md text-2xl"
                            data-bs-toggle="dropdown" href="#" aria-expanded="false">
...
var ctxs = document.getElementById("myAreaChartMonthly");
var myLineChartMonthly = new Chart(ctxs, {
    type: 'line',
    data: {
        labels: [],
        datasets: [{
    ...
    function setAreaMonthlyChartOption(label, ton) {
        // Mengubah data
    ...
    // Update AreaChart berdasarkan bulan dan minggu yang dipilih
        ...
        fetch(`/area-data?month=${month}&year=${year}`)
            ...
            .then(data => {
                ...
                if (data && data.labels && data.tons && data.monthlyLabels && data.monthlyTons) {
                    ...
                    setAreaMonthlyChartOption(data.monthlyLabels, data.monthlyTons.map(value => parseInt(value.replace(/\./g, ''), 10)));
                } else {
                ...
                if (data) { //Memastikan ada data di daily dan month
                    ...
                    tbody.innerHTML +=`
                        <tr>
                            ...
                            <td class="text-center text-xl">${data.daily[0] ? data.daily[0]['tons'] : '0'} ton</td>
                        </tr>
                        <tr>
                            <th style="display:flex; justify-content:center;">
                                <div class="text-xl" style="width:30%;">
                            ...
                            <td class="text-center text-xl">${data.monthly[0] ? data.monthly[0]['tons'] : '0'} ton</td>
                        </tr>
                    `;
                }
