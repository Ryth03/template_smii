<x-app-layout>
            ...
            <div class="inline-flex items-center">
                <nav>
                    <ol class="breadcrumb flex items-center">
                        ...
                        <li class="breadcrumb-item pr-1" aria-current="page">Location</li>
    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <h1 class="card-title text-3xl font-medium">List Locations</h1>
            </div>
            <div class="card-body">
                <div class="relative overflow-x-auto sm:rounded-lg">
                    <table id="locationsTable" class="table table-striped w-full  text-left rtl:text-right table-bordered">
                        <thead class="uppercase border-b">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-lg">#</th>
                                <th scope="col" class="px-6 py-3 text-lg">Location</th>
                                <th scope="col" class="px-6 py-3 text-lg">PIC</th>
                            ...
                            @foreach ($locations as $index => $location)
                                <tr>
                                    ...
                                    <td class="flex items-center justify-center text-xl space-x-4">
                                        <button type='button' data-modal-target="createRoleModal-{{ $location->id }}"
                                            data-modal-toggle="createRoleModal-{{ $location->id }}"
                                            class="text-fade btn btn-warning"><i
                                                class="fa-solid fa-pencil text-white"></i>
                                        </button>
                                        <form action="{{ route('location.destroy', $location->id) }}" method="POST" class="delete-form">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5">
                    <form class="space-y-4" action="{{ route('location.store') }}" method="POST">
                        @csrf
                        <div>
                            <label for="name"
                                class="block mb-2 text-xl font-medium text-gray-900 dark:text-white">Location Name</label>
                        ...
                        <div>
                            <label for="pic"
                                class="block mb-2 text-xl font-medium text-gray-900 dark:text-white">PIC</label>
                            <select name="pic" id="pic"
                                class="form-select text-md bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" 
                                required>
                                <option value="" disabled selected>-- Pilih PIC --</option>
                                @foreach($users as $user)
                                    <option value="{{ json_encode(['nik' => $user->nik, 'name' => $user->name]) }}">{{$user->name}} - {{$user->nik}}</option>
                                @endforeach
                            </select>
                        </div>
                        <button type="submit"
    ...
    @foreach ($locations as $location)
        <div id="createRoleModal-{{ $location->id }}" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button"
                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                            data-modal-hide="createRoleModal-{{ $location->id }}">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewbox="0 0 14 14">
                    ...
                    <!-- Modal body -->
                    <div class="p-4 md:p-5">
                        <form class="space-y-4" action="{{ route('location.update',$location->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div>
                            ...
                            <div>
                                <label for="pic" class="block mb-2 text-xl font-medium text-gray-900 dark:text-white">
                                    PIC
                                </label>
                                <input type="text" name="pic" id="pic"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white text-md"
                                    placeholder="PIC Name" required="" value="{{ $location->pic }}">
                            </div>
                            <div>
                                <label for="nik" class="block mb-2 text-xl font-medium text-gray-900 dark:text-white">
                                    NIK PIC
                                </label>
                                <input type="text" name="nik" id="nik"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white text-md"
                                    placeholder="NIK" required="" value="{{ $location->nik }}">
                            </div>
                            <button type="submit"
