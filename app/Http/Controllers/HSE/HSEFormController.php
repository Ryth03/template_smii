...
class HSEFormController extends Controller
{
    public function __construct(){
        ...
        $this->middleware('permission:create form hse', ['only' =>['createNewForm']]);
    ...
    public function viewList()
    {
        ...
        if($user){
            ...
            return view('hse.guest.dashboard', compact('forms'));
        }
        else{
    ...
    public function createNewForm(Request $request)
    {
        ...
        $severity_before  = $request->input('severityBefore');
        $opportunity_before = $request->input('opportunityBefore');
        $risk_factor_before = $request->input('riskFactorBefore');
        $danger_control = $request->input('dangerControl');
        $severity_after = $request->input('severityAfter');
        $opportunity_after = $request->input('opportunityAfter');
        $risk_factor_after = $request->input('riskFactorAfter');

        foreach ($potential_danger as $key => $potentialDanger) {
            jobSafetyAnalysis::create([
                'form_id' => $formId,
                'potential_danger' => $potential_danger[$key],
                'severity_before' => $severity_before[$key],
                'opportunity_before' => $opportunity_before[$key],
                'risk_factor_before' => $risk_factor_before[$key],
                'danger_control' => $danger_control[$key],
                'severity_after' => $severity_after[$key],
                'opportunity_after' => $opportunity_after[$key],
                'risk_factor_after' => $risk_factor_after[$key]
            ]);
        }
    ...
    {   
        ...
        ->select('potential_danger', 'danger_control')
        ->get()
        ->toArray();
        ...
        foreach ($existingJsasPotentialDanger as $existingDanger) {
            if(!in_array($existingDanger, $newJsasPotentialDanger)){
                jobSafetyAnalysis::where('form_id', $formId)
                    ->where('potential_danger', $existingDanger["potential_danger"])
                    ->where('danger_control', $existingDanger["danger_control"])
                    ->delete();
            }
        }
